...
def get_num_layer_for_vit(var_name, num_max_layer):
    if var_name in ('backbone.split_head.cls_token', 'backbone.split_head.mask_token',
        ...
        return 0
    elif var_name.startswith('backbone.split_head.visual_embed'):
        return 0
    elif var_name.startswith('backbone.split_head.patch_embed'):
        return 0
    elif var_name.startswith('backbone.split_head.') or var_name.startswith(
            'backbone.split_head.'):
        try:
    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters():
            ...
            if len(param.shape) == 1 or name.endswith('.bias') or name in (
                    'pos_embed', 'backbone.split_head.cls_token', 'backbone.split_head.visual_embed'):
                group_name = 'no_decay'
                this_weight_decay = 0.
            else:
                group_name = 'decay'
